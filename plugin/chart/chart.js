...
const initChart = function(Reveal){
  ...
  */
  ...
  var initializeSingleChart = function(canvas) {
    // check if canvas has data-chart attribute
    if ( canvas.hasAttribute("data-chart") ){
      var CSV = canvas.innerHTML.trim();
      var comments = CSV.match(/<!--[\s\S]*?-->/g);
      CSV = CSV.replace(/<!--[\s\S]*?-->/g,'').replace(/^\s*\n/gm, "")
      if ( ! canvas.hasAttribute("data-chart-src") ) {
        createChart(canvas, CSV, comments);
      }
      else {
        var canvas = canvas;
  ...
  var observer = new IntersectionObserver((entries) => {
    entries.forEach((entry) => {
      if (entry.isIntersecting) {
        initializeSingleChart(entry.target);
      } else {
        destroyChart(entry.target);
      }
    })
  });
